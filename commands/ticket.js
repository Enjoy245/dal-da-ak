...
.setTitle(`Archsolid - Destek`)
.setFooter(`archsolid.com`, client.user.avatarURL)
.setColor('GOLD')
.setDescription(`📩 Tepkisine tıklayıp destek kanalı açabilirsiniz.`)).then(m => {
m.react('📩')
...
s.send(`${author}, Destek Talebi Açtı | @here`, new Discord.RichEmbed()
.setColor('GOLD')
.setDescription(`Yakın bir zamanda Yetkililerimiz seninle ilgilenecektir.
Destek Talebini kapatmak istersen 🔒 Tepkisine tıkla!`)
.setFooter(`archsolid.com`, client.user.avatarURL)).then(m => {
m.react(`🔒`)
...
:no_entry:: Ticketi Tamamen Kapatır.`)).then(m2 => {
m2.react('🔓')
m2.react('⛔')
let sil = (reaction, user) => reaction.emoji.name === "⛔" && user.id !== client.user.id
...
s.send(new Discord.RichEmbed()
.setColor('GREEN')
.setDescription(`Ticket ${author} tarafından tekrar açıldı.`))
s.setName(`ticket-${as}`)
})
...
s.send(new Discord.RichEmbed()
.setColor('RED')
.setDescription(`Destek Kanalı **5 saniye** sonra kendini imha edecektir.`))
setTimeout(async () => {
s.delete()
const sd = await data.fetch(`ass.${message.guild.id}.${author.id}`)
